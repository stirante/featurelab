...
The fixtures behind this page are committed under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features): `multipart_dripstone_spike.json` (the worked example) and the three `multipart_panel_*.json` scenes behind [the heights figure](#the-four-roles), with their own `multipart_panel_column_*.json` columns and the shared `multipart_panel_anchor.json` / `multipart_panel_anchor_block.json` that lay the stone cube under each one.
