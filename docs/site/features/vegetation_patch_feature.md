...
The example fixtures are committed under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features): `vegetation_patch_floor.json` for the worked example, and `vegetation_patch_ceiling.json` with its `hanging_roots_ceiling_block.json` delegate and the `vegetation_patch_ceiling_demo.json` scaffolding for the ceiling example. The figure's own scenes are in the documentation's pack, [`docs/wiki/tools/figure-fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/figure-fixtures/features) — `veg_panel_floor.json` and `veg_panel_ceiling.json` — two aggregates that lay the two plates and then run `veg_panel_patch_floor.json` / `veg_panel_patch_ceiling.json`, which differ in the word `floor` and nothing else — over the shared `veg_panel_slab_grid.json` plate and the `threshold_marker.json` delegate. `featurelab check` is clean on all of them.
