...
The fixtures behind every table in this section are committed. Under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features): the three `wiki:rng_scatter_*` features — `rng_scatter_bare.json`, `rng_scatter_degenerate.json`, `rng_scatter_nondegenerate.json` — their shared `wiki:rng_marker` delegate (`rng_marker.json`), and the two `rng_scatter_evalorder_*.json` files. In the documentation's own pack, [`docs/wiki/tools/figure-fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/figure-fixtures/features): the six `dist_*.json` files and their `dist_panel_*.json` centring wrappers behind the distribution figure, with a copy of `rng_marker.json` alongside them, because a pack has to resolve its own delegates.
