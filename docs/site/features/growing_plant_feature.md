...
The fixtures are committed in two packs: `growing_plant_cave_vines.json` for the worked example under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features), and the figure's own scenes under [`docs/wiki/tools/figure-fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/figure-fixtures/features) — `growplant_panel_up.json` and `growplant_panel_down.json` — two aggregates that lay the two plates and then run `growplant_panel_vine_up.json` / `growplant_panel_vine_down.json`, which differ in the word `up` and nothing else — over the shared `growplant_panel_plate_grid.json` plate. `featurelab check` is clean on all of them.
