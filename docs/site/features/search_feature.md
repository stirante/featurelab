...
`search_pumpkin_down.json` and `search_pumpkin_too_shallow.json`, the fixtures behind the example, are committed under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features). The six `search_panel_*.json` files behind the figure are in the documentation's own pack, [`docs/wiki/tools/figure-fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/figure-fixtures/features), along with a copy of their shared `threshold_marker.json` delegate — a pack has to resolve its own delegates, so that one file is committed in both.
