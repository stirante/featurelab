...
Every measured claim on this page is reproducible from committed fixtures. `tree_acacia_branching.json` (the worked example), `tree_plain_trunk.json`, `tree_fancy_oak.json` and `tree_poplar.json`, behind the single images, are under [`docs/wiki/tools/fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/fixtures/features); the eight `tree_panel_*.json` files behind [the trunk-kinds figure](#the-eight-trunk-keys) are in the documentation's own pack, [`docs/wiki/tools/figure-fixtures/features/`](https://github.com/stirante/featurelab/tree/main/docs/wiki/tools/figure-fixtures/features).
